...
$(document).ready(function () {
	...
	if (!minimize_sidebar) {
		...
		minibutton.on('click', function () {
			...
			receiveAjaxRequest(`/core/config/set_config/sidebar_minimize/${mini_sidebar}/`);
...
// Marcar a opção do menu lateral ativa com base na url atual
function setActiveNavLink() {
	const currentUrl = window.location.pathname;

	// Considerar somente os dois primeiros segmentos: /app/model
	const currentBase = currentUrl.split("/").slice(0, 3).join("/");
	const links = document.querySelectorAll('.sidebar-content a');

	links.forEach(link => {
		const linkPath = new URL(link.href).pathname;
		const linkBase = linkPath.split("/").slice(0, 3).join("/");
		...
		if (currentBase === linkBase) {
			...
			if (parentLi) {
				...
				// Se o menu for aninhado, ativa os pais também
...
function removeModalFocusOnClose() {
...
function initTemplate() {
	try {
		setActiveNavLink();
		removeModalFocusOnClose();
	} catch (error) {
		console.error('Não foi possível inicializar o template', error);
	}
}